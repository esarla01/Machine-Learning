...
class InternalDecisionNode(object):
    ...
    def predict(self, x_TF):
        ...
        for idx in range(T):
            if x_TF[idx][self.feat_id] <= self.thresh_val:
                left_mask.append(idx)
            else:
                right_mask.append(idx)
        # Ask the left child for its predictions (call 'predict')
        left_pred = self.left_child.predict(x_TF[left_mask])
        # Ask the right child for its predictions (call 'predict')
        right_pred = self.right_child.predict(x_TF[right_mask])

        # Aggregate all predictions and return one array
        # Hint: Make sure to preserve the order of examples as in the input.
        yhat_T = np.ones(T, dtype=np.float64)
        yhat_T[left_mask] = left_pred
        yhat_T[right_mask] = right_pred
        return yhat_T
