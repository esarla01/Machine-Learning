...
def calc_root_mean_squared_error(y_N, yhat_N):
    ...
    # Calculate squared differences between true values (y_N) and predictions (yhat_N)
    squared_error_per_sample = np.square(y_N - yhat_N)

    # Calculate the total sum of squared errors
    total_squared_error = np.sum(squared_error_per_sample)

    # Calculate the Root Mean Squared Error (RMSE)
    root_mean_squared_error = np.sqrt(total_squared_error / col_num)
