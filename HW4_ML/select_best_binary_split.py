...
def select_best_binary_split(x_NF, y_N, MIN_SAMPLES_LEAF=1):
    ...
    for f in range(F):
        ...
        for v in range(V):
            # Determine which samples go to the left and right child
            left_mask_N = x_NF[:, f] < possib_xthresh_V[v]
            right_mask_N = np.logical_not(left_mask_N)

            # Calculate the mean prediction for each child
            if np.any(left_mask_N):  # checks if any of the values is True
                left_yhat_V[v] = np.mean(y_N[left_mask_N])
            else:
                left_yhat_V[v] = 0

            if np.any(right_mask_N):
                right_yhat_V[v] = np.mean(y_N[right_mask_N])
            else:
                right_yhat_V[v] = 0

            # Calculate the cost for each child
            left_cost_V[v] = np.sum((y_N[left_mask_N] - left_yhat_V[v]) ** 2)
            right_cost_V[v] = np.sum(
                (y_N[right_mask_N] - right_yhat_V[v]) ** 2)

            # Total cost for the current threshold
            total_cost_V[v] = left_cost_V[v] + right_cost_V[v]
    ...
    # print('THRESHOLD VALUE: ', best_thresh_val)
