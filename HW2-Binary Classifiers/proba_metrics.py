...
def calc_mean_binary_cross_entropy_from_probas(ytrue_N, yproba1_N):
    ...
    if (len(ytrue_N) == 0 or len(yproba1_N) == 0):
        return 0
    mean_bce = 0.0
    ...
    for index in range(N):
        y_n = ytrue_N[index]
        p_n = yproba1_N[index]
        bce = ((-1) * y_n * np.log2(p_n)) - ((1 - y_n) * np.log2(1 - p_n))
        mean_bce += bce
...
def calc_mean_binary_cross_entropy_from_scores(ytrue_N, scores_N):
    ...
    True
    '''
    if (len(ytrue_N) == 0 or len(scores_N) == 0):
        return 0.0
    ...
    flipped_scores_N = yflippedsign_N * scores_N  # implement flip(y_n) s_n

    # Stack two arrays of shape (N,1) to get [0, flipped_scores_N]
    scores_and_zeros_N2 = np.column_stack((np.zeros(N, ), flipped_scores_N))

    # Get the value of the expression log_e(2)
    log_e_2 = np.log(2) / np.log(np.exp(1))

    # Be sure to use scipy_logsumexp from scipy to handle 2D arrays and handle empty
    bce_score = 0
    for indx in range(N):
        log_sum_exp = scipy_logsumexp(scores_and_zeros_N2[indx])
        log_sum_exp = log_sum_exp / log_e_2
        bce_score += log_sum_exp

    bce_mean = bce_score / N
